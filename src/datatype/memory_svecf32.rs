...
impl SVecf32Output {
    pub fn new(vector: SVectBorrowed<'_, f32>) -> SVecf32Output {
        unsafe {
            ...
            (&raw mut (*ptr).varlena).write(SVecf32Header::varlena(layout.size()));
            (&raw mut (*ptr).reserved).write(0);
            (&raw mut (*ptr).magic).write(HEADER_MAGIC);
            (&raw mut (*ptr).dims).write(vector.dims());
            (&raw mut (*ptr).len).write(vector.len());
...
impl IntoDatum for SVecf32Output {
    ...
    fn type_oid() -> Oid {
        ...
        let t = pgrx_catalog::PgType::search_typenamensp(c"svector", namespace.oid()).unwrap();
