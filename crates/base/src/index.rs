    ...
    fn validate_self_quantization(
        ...
        match quantization {
            ...
            QuantizationOptions::Rabitq(_) => {
                if !matches!(self.vector.v, VectorKind::Vecf32 | VectorKind::Vecf16) {
                    return Err(ValidationError::new(
                        "RaBitQ quantization is not support for vectors that are not dense vectors",
                    ));
                }
                if !matches!(self.vector.d, DistanceKind::L2) {
                    return Err(ValidationError::new(
                        "RaBitQ quantization is not support for distance that is not squared Euclidean distance",
                    ));
                }
                Ok(())
            }
