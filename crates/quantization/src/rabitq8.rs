...
#[serde(bound = "")]
pub struct Rabitq8Quantizer<O: OperatorRabitq8Quantization> {
    dims: u32,
    projection: Vec<Vec<O::Scalar>>,
    _maker: PhantomData<O>,
}

impl<O: OperatorRabitq8Quantization> Quantizer<O> for Rabitq8Quantizer<O> {
    fn train(
...
pub trait OperatorRabitq8Quantization: Operator {
...
impl<S: ScalarLike> OperatorRabitq8Quantization for VectL2<S> {
    type Scalar = S;
...
impl<S: ScalarLike> OperatorRabitq8Quantization for VectDot<S> {
    type Scalar = S;
...
macro_rules! unimpl_operator_rabitq_quantization {
    ($t:ty) => {
        impl OperatorRabitq8Quantization for $t {
