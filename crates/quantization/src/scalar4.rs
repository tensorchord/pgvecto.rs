...
#[serde(bound = "")]
pub struct Scalar4Quantizer<O: OperatorScalar4Quantization> {
    dims: u32,
    _maker: PhantomData<O>,
}

impl<O: OperatorScalar4Quantization> Quantizer<O> for Scalar4Quantizer<O> {
    fn train(
...
pub trait OperatorScalar4Quantization: Operator {
...
impl<S: ScalarLike> OperatorScalar4Quantization for VectL2<S> {
    type Scalar = S;
...
impl<S: ScalarLike> OperatorScalar4Quantization for VectDot<S> {
    type Scalar = S;
...
macro_rules! unimpl_operator_rabitq_quantization {
    ($t:ty) => {
        impl OperatorScalar4Quantization for $t {
